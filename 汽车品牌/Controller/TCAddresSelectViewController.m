...
- (void) setSearchControllerHidden:(BOOL)hidden {
    ...
    NSInteger height = hidden ? 0: _tbView.frame.size.height;
  
   [self.searchResultTbView setFrame:CGRectMake(0, _tbView.frame.origin.y, ScreenWidth, height)];
...
{
    self.title = @"定位";
    [self.navigationController.navigationBar setBarTintColor:HWColor(225, 63, 48)];
    if (iOS7)
    {
    ...
    self.navigationItem.leftBarButtonItem=[[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"My-order_nav_icon_back_n"]style:UIBarButtonItemStylePlain target:self action:@selector(BackButtonClick)];
    ...
    self.searchResultTbView = [[UITableView alloc] initWithFrame:CGRectMake(0, _tbView.frame.origin.y, ScreenWidth, 0) style:UITableViewStylePlain];
    
   // self.searchResultTbView = [[UITableView alloc] initWithFrame:myRect(0, _tbView.frame.origin.y, ScreenWidth, 0) style:UITableViewStylePlain];
...
- (void)headerLabelTap:(UITapGestureRecognizer *)tap
{
    ...
    view.backgroundColor = HWColor(220, 220, 220);
...
- (void)loationMangerFaildWithError:(NSError *)error
{
    ...
    else if ([error code] == 1)
    {
        if (iOS8) {
            ...
            Alert(@"您未允许汇淘车访问您的位置,请到设置中设置允许访问位置");
        }
    }
