...
- (UIImageView *)headerImageView {
    if (!_headerImageView) {
        ...
        _headerImageView.size = CGSizeMake(80, 80);
        _headerImageView.center = CGPointMake(kSelfWidth / 2, kSelfHeight / 2);
        _headerImageView.layer.cornerRadius = kHeaderWidth / 3;
...
- (UILabel *)label {
    ...
    if (!_label) {
        ...
        _label.font = [UIFont systemFontOfSize:MyNavTitleSize];
        _label.textAlignment = 1;
        _label.size = CGSizeMake(kSelfWidth, 20);
        _label.centerY = kSelfWidth / 2;
        _label.top = self.headerImageView.bottom + 10;
...
-(UIButton *)button{
    ...
    if (!_button) {
        ...
        _button.font=[UIFont systemFontOfSize:MyNavTitleSize];
        _button.titleLabel.textAlignment=1;
        _button.size=CGSizeMake(kSelfWidth, 20);
        _button.centerY=kSelfWidth/2;
        _button.top=self.headerImageView.bottom + 10;
        [_button addTarget:self action:@selector(ChangeNameBtn:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark -
#pragma mark - 点击方法
-(void)ChangeNameBtn:(UIButton *)send{
